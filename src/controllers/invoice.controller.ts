...
import type { CreateInvoice, UpdateInvoice } from '../invoice.ts';
...
export function createInvoice(request: Request, response: Response) {
    const { value, customer_id, created_at } = request.body as CreateInvoice;

    const invoice = InvoiceService.insertInvoice({ value, customer_id, created_at });
...
export function updateInvoice(request: Request, response: Response) {
    const id = Number(request.params.id);
    const { value, customer_id, status, created_at } = request.body as UpdateInvoice;
